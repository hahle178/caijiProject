...
public class CreateXmlServiceImpl implements CreateXmlService {
    ...
    public void createIndexXml(String xmlPath, LinkedList bcpMessages, List<List<Object>> bankListByExcel, String SDateSet) throws IOException {
        ...
        //查询需要遍历的节点属性值
        List<DataMapping> dataMappings = dataMappingXmlService.selectBySDateSet(SDateSet);
        ...
        int len = 0;
        if (bankListByExcel.size() > 0) {
            for (List<Object> list : bankListByExcel) {

                Element DATA1 = root.createCopy();
                DATA1=DATASET1.addElement("DATA");
